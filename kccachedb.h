...
struct rcxlock {
  char __attribute__((aligned(RCX_PADDING)))
    pndlocks[RCX_PADDING * RCX_NR_NODES];
  pthread_spinlock_t __attribute__((aligned(RCX_PADDING))) glblock;
};

#define pndlockofnd(lock, nd) \
  ((lock)->pndlocks[RCX_PADDING * nd])

#define pndlockof(lock) \
  ((lock)->pndlocks[RCX_PADDING * numa_node_id])

#define rcx_pnd_locked(lock) \
  (ACCESS_ONCE(pndlockof(lock)) == 1)

#define rcx_pnd_lock(lock) \
  ACCESS_ONCE(pndlockof(lock)) = 1;

#define rcx_pnd_unlock(lock) \
  ACCESS_ONCE(pndlockof(lock)) = 0;

#define rcx_glb_lock(lock) \
  pthread_spin_lock(&lock->glblock);

#define rcx_glb_unlock(lock) \
  pthread_spin_unlock(&lock->glblock);

#define rcx_begin \
  if (_xbegin() == (int)_XBEGIN_STARTED)
...
static inline void rcx_lock_init(struct rcxlock *lock)
{
  ...
  for (nd = 0; nd < RCX_NR_NODES; nd++)
    ACCESS_ONCE(pndlockofnd(lock, nd)) = 0;
  pthread_spin_init(&lock->glblock, PTHREAD_PROCESS_PRIVATE);
}
...
class CacheDB : public BasicDB {
 ...
 public:
  ...
  bool accept_ro(const char* kbuf, size_t ksiz, Visitor* visitor, bool writable = true) {
	  uint64_t version;
...
fallback:
  ...
  bool accept(const char* kbuf, size_t ksiz, Visitor* visitor, bool writable = true) {
    ...
    hash /= SLOTNUM;
    Slot* slot = slots_ + sidx;
#ifdef  CONFIG_USE_RCX
    rcx_lock(&slot->rcxlock);
#else
    slot->lock.lock();
#endif
    slot->version++;
